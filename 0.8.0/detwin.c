...
void stat_pearson_i_sp(RefList *list1, RefList *list2, double * val, const SymOpList *sym)
{
	double *vec1, *vec2;
	double *vec3, *vec4;
	int ni = num_reflections(list1); // bug here. TODO
	int nacc = 0;
	int nacc_twin = 0;
	Reflection *refl1;
	RefListIterator *iter;
		...
		refl1 = next_refl(refl1, iter) )
	{
		...
		if ( refl2 != NULL ) /* This is a common reflection */
		{
...
void merge_image_to_model( RefList* model, RefList *image, double* cc, int iter, const SymOpList *sym, double min_snr )
{
	int twin;
...
void emc( RefList **image_array, RefList* full_list, const SymOpList *sym, int n_image, int max_n_iter, int WINNER_TAKES_ALL, double min_snr )
{
	int image_index;
	int winner, n_iter;
	RefList *image_a;
...
int main(int argc, char *argv[])
{
	...
	full_list = make_reflections_for_uc_from_asymm( model, false, sym );

	printf("\nExpectation Maximization\n");
	emc( image_array, full_list, sym, n_crystals_recorded, max_n_iter, WINNER_TAKES_ALL, min_snr );
